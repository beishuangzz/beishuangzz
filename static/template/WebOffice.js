function WebOffice2015() {
	// this.dVersion = "4.8.8.07";
	// this.dUpdateDate = "2018-8-07";
	this.dVersion = "4.8.7.26";
	this.dUpdateDate = "2018-7-26";
	//高级功能：全文检索、文档对比
	...
	this.OfficeUserName = ""; ////记录本地office用户名 （该功能已完整实现）
	this.FileType = ""; // 文档类型 .doc .xls .wps		（该功能已完整实现）
	this.EditType = ""; //文档编辑类型					（该功能已完整实现）
	...
	this.UIControl = false; // 控制OFFICE2010环境中是否可以使用另存为和保存功能，用于控制客户使用自定义快捷键和快捷工具栏的另存为功能下载文件	（该功能已完整实现）
	...
	// 保存HTML到服务器        （该功能已完整实现）  需要V12.4.0.450以上版本
	this.WebSaveAsHtml = function()
	{
		this.ConsoleOut("<WebSaveAsHtml> 开始...");
		if (this.SaveAsHtml(this.getFilePath() + this.RecordID + ".html")) {
			this.ConsoleOut("<WebSaveAsHtml> Html保存到本地成功...");
			var strDirPath = this.getFilePath() + this.RecordID + '.files'; //获取html files路径
			//var DirPath = new VBArray(this.obj.FileSystem.FindToDirAsFile(strDirPath)); //获取html文件夹里的内容
			...
			this.ConsoleOut("<WebSaveAsHtml> 循环保存files文件夹内容...");
			...
			this.ConsoleOut("<WebSaveAsHtml> files文件夹内容成功,开始保存Html文件夹内容...");
			if (this.SAVEFILE(httpclient, this.getFilePath() + this.RecordID + ".html")){
				this.ConsoleOut("<WebSaveAsHtml> 保存Html文件夹内容成功...");
		...
		this.ConsoleOut("<WebSaveAsHtml> 保存Html文件夹内容失败...");
		this.Status = "保存HTML文件失败!";
	...
	this.WebSaveImage = function (ImageType, ImageName)
	{
		...
		if(this.FileType == ".doc" || this.FileType == ".docx" || this.FileType == ".wps"){
			...
			this.ConsoleOut("<WebSaveImage> Html保存到本地成功...");
			...
			this.WebSetMsgByName("DIRECTORY", this.RecordID + ".files");
			this.ConsoleOut("<WebSaveImage> 循环保存files文件夹内容...");
			for(var i=0; i<DirPath.length;i++){
				var FilesName = DirPath[i];
				this.WebSetMsgByName("FILENAME", FilesName);// 加载FileName
				if (!this.SAVEFILE(httpclient, strDirPath + "\\" + FilesName)){
			...
			this.ConsoleOut("<WebSaveImage> files文件夹内容成功,开始保存Html文件夹内容...");
			if (this.SAVEFILE(httpclient, this.getFilePath() + this.RecordID + ".htm")){
				this.ConsoleOut("<WebSaveImage> 保存Html文件夹内容成功...");
		...
		this.ConsoleOut("<WebSaveImage> 保存Html文件夹内容失败...");
		this.Status = "保存HTML图片文件失败!";
		this.ConsoleOut("<WebSaveImage> 结束...");
		return false;
	}
    //  插入服务器上执行的文档   （该功能已完整实现、已做修改）
	...
	this.WebOpenBookMarks = function()
	{
		if(this.FileType == ".doc" || this.FileType == ".docx"){
			if(this.obj.ActiveDocument.ProtectionType == "-1"){
				var FunctionString = "ActiveObject.ActiveDocument.Application.Dialogs.Item(168).Show()";
	...
	this.WebLoadBookMarks = function() {
        ...
        if(httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)){
        	...
        	if(httpclient.Send()){
        		this.ConsoleOut("<WebLoadBookMarks> 获取书签内容插入到书签...");
        		...
        		try {
        			for(var i  in jsonObj){
        				...
        				if(this.obj.ActiveDocument.BookMarks.Exists(BookName)){
        					...
        					this.ConsoleOut("<WebLoadBookMarks> 没有找到" + BookName + "书签...");
        					this.Status = "没有找到" + BookName + "书签";
        					return false;
        				}
        			}
        		} catch (e) {
        			this.ConsoleOut("<WebLoadBookMarks> 插入书签出现异常" + e.description);
        			this.Status = e.description;
        			return false;
        		}
        		this.ConsoleOut("<WebLoadBookMarks> 书签内容已插入到书签...");
        		this.Status = "书签内容已插入到书签";
                return true;
        	}else{
        		this.ConsoleOut("<WebLoadBookMarks> 向后台发送数据包错误...");
        		this.Status = "向后台发送数据包错误";
        		return false;
        	}
        }else{
        	this.ConsoleOut("<WebLoadBookMarks> 打开链接错误...");
        	this.Status = "打开链接错误";
    		return false;
        }
	}
...
/*		if(fs.ClearDirectory(DirName)){
    ...
    this.WebReadString = function(mKey)
    {
    	...
    	if(root != undefined && rootValue != undefined){
    		...
    		if(res != null || res != ""){
    			...
    			this.ConsoleOut("<WebReadString> 读取失败，没有对应的注册表...");
    ...
    {
    	this.ConsoleOut("<WebWriteInteger> 开始...");
    	var register = this.obj.Register;
    	if(root != undefined && rootValue != undefined){
    		this.ConsoleOut("<WebWriteInteger> 开始写入注册表项的值...");
    		var res = register.SetDWordValue(root, rootValue, mKey, mValue);
    		this.ConsoleOut("<WebWriteInteger> 写入注册表成功...");
    		this.Status = "写入注册表成功";
    ...
    this.ClearRevisions = function()
    {
    	if (this.getDocType(this.FileType) == this.DocType.WORD && this.obj.ActiveDocument.ProtectionType == "-1") {
    		...
    		this.ConsoleOut("非Word文档或文档已锁定，操作失败");
			//this.Status = "痕迹接受失败，错误原因：" + e.description;
			return false;
    	}
	...
	// 文档对比	（该功能已完全实现）
	this.WebDocumentCompare = function(FileName1,FilePath1,FileName2,FilePath2)
	{
		// 与当前打开的文档进行对比
		...
		if ((FileName1 == null) || (FileName1 == '')) {
			if ((FileName2 == null) || (FileName2 === '')) {
				...
				if (this.WebDownloadFile_int(FileName2,FilePath2)) {
		...
		// 与指定的后台两个文档进行对比
			...
			if ((FileName2 == null) || (FileName2 == '')) {
				...
				if (this.WebDownloadFile_int(FileName1,FilePath1)) {
					...
					if ((this.WebDownloadFile_int(FileName2,FilePath2))) {
						...
						if (bRet == true) {
							...
							this.NewShowMenu(this.ShowMenu);  //控制菜单栏是否可以显示
							this.NewShowToolBar(this.ShowToolBar); //控制Office工具栏和自定义工具栏
							this.obj.ActiveDocument.TrackRevisions = true; // 显示标记和隐藏标记
							this.obj.ActiveDocument.ShowRevisions = true; // 显示痕迹或隐藏


							//console.log(this.obj.ActiveDocument.Revisions);
							//console.log(this.obj.ActiveDocument.Revisions.Item[0]);
							//this.obj.ActiveDocument.Application.UserName = "111";
	...
	/*控制2015菜单栏*/		// （该功能已完整实现）
    this.NewShowMenu = function(mValue)
    {
    	var style = this.obj.Style;
    	switch(mValue)
    	{
    ...
    this.NewShowToolBar = function(mValue)
    {
    	...
    	switch(mValue)
    	{
    		case false : style.ShowCustomToolbar = false; style.ShowToolBars = true; break; //自定义工具栏隐藏，office工具栏显示
    		case true : style.ShowCustomToolbar = true; style.ShowToolBars = true; break; //自定义工具栏显示，office工具栏显示
    		case 0: style.ShowCustomToolbar = false; style.ShowToolBars = true; break; //自定义工具栏隐藏，office工具栏显示
    		case 1: style.ShowCustomToolbar = true; style.ShowToolBars = true; break; //自定义工具栏栏显示，office工具栏显示
  ...
  //禁用修订，更正
    ...
    //禁用修订菜单
    this.RemoveRevise = function()
    {
		var dealcount = 0;
		var controls = this.obj.ActiveDocument.Application.CommandBars.Item("Track Changes").Controls;
	    for ( i = 1 ; i <= controls.Count; i++ )
	    {
		     if(dealcount == 3){
		     	return;
		     }
		     var menuid = controls.Item(i).ID;
		     if( menuid == 1715 ){
    ...
    */
			...
			if ((this.FileType == ".doc") || (this.FileType == ".docx")
				...
				try {
					...
					/*this.obj.ActiveDocument.ExportAsFixedFormat(FilePath, 17,
							...
							this.ConsoleOut("<WebSavePDF> 保存PDF到本地成功...");
				...
				try {
					this.obj.ActiveDocument.Application.ActiveWorkbook.ExportAsFixedFormat(0,FilePath,0);
							this.ConsoleOut("<WebSavePDF> 保存PDF到本地成功...");
							this.Status = "保存PDF到本地成功";
							return true;
				} catch (e) {
				...
				this.ConsoleOut("<WebSavePDF> 保存PDF异常,文档格式不对");
			}
		}else{
	...
	this.WebDownloadFile_int = function(fn,filePath)
	{
		this.Status = '';
		var httpclient = this.obj.Http; // 设置http对象
		httpclient.Clear();
		...
		this.WebSetMsgByName("GETFILEPATH", filePath); // 加载FileName
		this.WebSetMsgByName("FILETYPE", this.FileType); // 加载FileType
		this.WebSetMsgByName("RECORDID", this.RecordID); // 加载RecordID
		this.WebSetMsgByName("OPTION", "LOADFILE"); // 发送请求LOADFILE
		httpclient.AddForm("FormData", this.GetMessageString()); // 这里是自定义json
			...
			if (httpclient.Send()) {
				...
				if (this.hiddenSaveLocal(httpclient, this, false, false)) {
					...
					if (mSaveResult == 0) { // 打开本地磁盘文件
						...
						this.ConsoleOut("<LOADFILE> 打开文档时未知错误！错误码为： "
										+ mSaveResult);
	...
	this.AddWaterMark = function(WaterMarkNmae){
		if(this.FileType == ".doc" || this.FileType == ".docx"){
			...
			let intPage = parseInt(intPageTotal);
			...
			if(this.obj.ActiveDocument.ProtectionType == "-1"){
				...
				try {
					...
					selection.HeaderFooter.Shapes.AddTextEffect(0, WaterMarkNmae, '宋体', 36, false, false, 100,100).Select();
					...
					shapeRange.Fill.Transparency = 0.9;				//设置透明度50%
					shapeRange.Rotation = 60;									//设置旋转角度
		...
		if(this.FileType == ".doc" || this.FileType == ".docx"){
			...
			if(this.obj.ActiveDocument.ProtectionType == "-1"){
				...
				try {
					...
					shapeRange.Height = 5 * 72; //高度
					shapeRange.Width = 5 * 72; //宽度
/*					//doc 和 docx坐标不一样，修改需要等比例放大，这里的比例是我通过视觉大致调整的，具体原因未知
					if(this.FileType == ".doc"){
						shapeRange.Height = 3.4 * 3 * 76; //高度
						shapeRange.Width = 3 * 76; //宽度
					}else{
						shapeRange.Height = 1 * 72; //高度
						shapeRange.Width = 3.4 * 72; //宽度
					}*/
   ...
   //自定义工具栏按钮是否显示（该功能已经完全实现）
	...
	this.DownloadToFile = function(DownFileName, SavePathName)
	{
		var httpclient = this.obj.Http;
		var URL = this.WebUrl.substring(0, this.WebUrl.lastIndexOf("/"));
		console.log(URL + "/law/common/file/downloadWaterMarker/"+ DownFileName);
		httpclient.ShowProgressUI = this.ShowWindow;// 隐藏进度条
		if (httpclient.Open(this.HttpMethod.Get, URL + "/law/common/file/downloadWaterMarker/"+ DownFileName, false)) {// 指定下载模板的名称
			if (httpclient.Send()) {
				if (httpclient.Status == 200) {
    ...
    this.OleDialogPrint =function(OLEFlag, bCancel){
    	 if (OLEFlag == 1) {
    	        ...
    	        exts += "|All File(*.*)|*.*";
    	        exts += "||";
    	        WebOffice.obj.ShowDialog(1, exts, 0);
    	    }
    	    else if (OLEFlag == 4) {
    	        ...
    	        WebOffice.obj.ShowDialog(4);
    	    }
    	    else if (OLEFlag == 32) {
    	        WebOffice.obj.ShowDialog(5);
    	    }
    	    else if (OLEFlag == 64) {
    	        WebOffice.obj.ShowDialog(6);
    	    }
    	    else if (OLEFlag == 128) {
    	        WebOffice.obj.ActiveDocument.PrintPreview();
    	    }
    }
}
